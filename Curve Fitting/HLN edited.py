...
# =========================
try:
    data = np.loadtxt("CBST2_1.95K_full data.csv", delimiter=",", skiprows=1)
    B = data[:, 0]          # Magnetic field (T)
    Y = data[:, 1]          # ΔG_norm (already normalized)
except Exception as e:
    print("Error loading CSV file:", e)
    exit()

# Estimate noise for chi-square (rough)
...
# HLN for NORMALIZED ΔG
# Prefactor A is NOT physical alpha
...
MODELS = {
    ...
    "HLN (normalized ΔG)": (HLN_model_norm, [0.05, 100e-9], ["A", "lphi"])
}
...
def run_fit():
    ...
    try:
        popt, pcov = curve_fit(func, B, Y, p0=p0, maxfev=20000)
    except Exception as e:
    ...
    # =========================
    # Build result string (CLEAR & SAFE)
    # =========================
    ...
    for name, val in zip(param_names, popt):
        ...
        elif name.lower() == "a" and model_name.startswith("HLN"):
    ...
    # Also print to terminal (useful for logging)
    print("========== FIT RESULT ==========")
    print(result)
    print("================================")
...
# =========================
root = tk.Tk()
root.title("Origin-like Nonlinear Fitting (with HLN, normalized ΔG)")
